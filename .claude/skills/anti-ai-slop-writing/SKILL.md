...
## Applying this to reference documentation

The rules above target prose — overviews, guides, marketing copy, narrative explanation.
Structured reference material (parameter tables, schema fields, CLI flag lists) follows
technical-writing convention instead: plain declarative statements, standard passive
constructions where the actor is genuinely irrelevant ("the value is tokenized"), and tables
over prose where a table is clearer. Don't force reference tables into a "human voice" — apply
the structural and vocabulary rules to the sentences and paragraphs around them.
